function mfcc =mfcc_extraction(wav)
%%
%% Compute Mel Frequency Cepstrum Coefficients.
%%   [mfcc,DCT] = mfcc_extraction(wav, p)
%%
%% USAGE
%%   ma_mfcc;                         %% test mode
%%   mfcc = mfcc_extraction(wav, p);          %% normal usage
%%   [mfcc, DCT] = mfcc_extraction(wav, p);   %% DCT matrix is needed to decode the 
